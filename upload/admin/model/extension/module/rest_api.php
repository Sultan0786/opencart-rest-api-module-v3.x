...
class ModelExtensionModuleRestApi extends Model
{
	...
	public function addApi($post)
	{
		$result = $this->db->query("
			...
			SET `title` = '" . $post['title'] . "', `public_key` = '" . $post['public'] . "', `private_key` = '" . $post['private'] . "', `status` = '" . $post['module_rest_api_status'] . "', `created_at` = NOW()
		");

		if ($result) {
			$this->moduleStatusUpdate($post['module_rest_api_status']);
			return true;
		} else {
			return false;
		}
	...
	public function moduleStatusUpdate($status)
	{
		$this->load->model('setting/setting');
		$settings['module_rest_api_status'] = $status;
