...
class ModelExtensionModuleRestApi extends Model
{
	...
	public function getOrders($custom_fields)
	{
		$select_query_fields = "*";

		if (!empty($custom_fields)) {
			$select_query_fields = $custom_fields;
		}

		try {
			$sql = "SELECT ".$select_query_fields." FROM " . DB_PREFIX .$this->_orders_table . $this->where." ORDER BY order_id DESC";
	...
	public function getOrdersStatus($order_ids)
	{	
		try {

			$sql = "SELECT os.name as status FROM " . DB_PREFIX . "order_status os INNER JOIN " . DB_PREFIX . "order o ON os.order_status_id = o.order_status_id WHERE o.order_id IN ($order_ids)";
