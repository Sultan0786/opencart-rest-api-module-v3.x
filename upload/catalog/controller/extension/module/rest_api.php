...
class ControllerExtensionModuleRestApi extends Controller
{
	...
	private $json            = [];
	private $filters         = [];
	private $methods         = ['getOrders' => 'GET', 'getProducts' => 'GET', 'getOrderStatus' => 'GET', 'update' => 'POST', 'updateInventory' => 'POST', 'updatePrice' => 'POST', 'validate' => 'GET', 'getProduct' => 'GET','getCancelOrders'=>'GET', 'getSelectedOrderStatus'=>'GET'];

	public function __construct($registry)
	{
		parent::__construct($registry);
		$this->sendErrorWhenModuleIsDisable();
	}
	...
	private function getOrders()
	{
		...
		$orders_custom_fields = isset($this->filters['custom_fields']) && !empty($this->filters['custom_fields']) ? $this->filters['custom_fields'] : null;
		unset($this->filters['custom_fields']);

		$orders = $this->model_extension_module_rest_api->setFilters($this->filters)
														->getOrders($orders_custom_fields);
	...
	public function sendErrorWhenModuleIsDisable()
	{
		if(!$this->config->get('module_rest_api_status')){
			$this->response->addHeader('HTTP/1.1 403 the requested resource is forbidden');
			$this->errors[] = "module should be enable!";
		}
	}

	private function getCancelOrders()
	{
		$this->load->model('account/order');
		...
		foreach ($orders as $key => $order) {
			...
			$orders[$key]['status'] = $this->getComment($order['order_status_id']);
		}

		$this->json = $orders;
	}

	private function getSelectedOrderStatus()
	{
		$this->load->model('account/order');

		$order_ids = isset($this->filters['order_id']) && !empty($this->filters['order_id']) ? $this->filters['order_id'] : null;

		$result = $this->model_extension_module_rest_api->getOrdersStatus($order_ids);
